  ...
  if (type == "classification") {
    ...
    if (!is_not(cost) && !is_not(margin)) {
      ...
      if (any(loss1 <= 0)) {
        ...
        stop("Cost must be larger than zero. Please adjust the settings and try again" %>% add_class("crtree"))
      } else {
        parms[["loss"]] <- matrix(c(0, loss1, loss2, 0), nrow = 2, byrow = TRUE)
      }
    } else if (!is.empty(prior)) {
      if (!all(is.numeric(prior))) {
        stop("Prior did not resolve to a numeric factor" %>% add_class("crtree"))
      } else if (any(prior > 1 | prior < 0)) {
        stop("Prior is not a valid probability" %>% add_class("crtree"))
      } else {
