...
crtree <- function(dataset, rvar, evar, type = "", lev = "", wts = "None",
  ...
  if (rvar %in% evar) {
    stop("Response variable contained in the set of explanatory variables. Please update model specification."
         %>%
           add_class("crtree"))
  }

  if (is.empty(cp)) {
    stop("Please provide a complexity parameter to split the data."%>%
           add_class("crtree"))
  } else if (!is.empty(nodes) && any(nodes < 2)) {
    stop("The (maximum) number of nodes in the tree should be larger than or equal to 2."%>%
           add_class("crtree"))
  }
  ...
  if (!is.empty(wts)) {
    ...
    if (length(wts) != nrow(dataset)) {
      stop(paste0("Length of the weights variable is not equal to the number of rows in the dataset (", format_nr(length(wts), dec = 0), " vs ", format_nr(nrow(dataset), dec = 0), ")")%>%
             add_class("crtree"))
    }
  }
  ...
  if (is.factor(dataset[[rvar]])) {
    if (type == "regression") {
      stop("Cannot estimate a regression when the response variable is of type factor."%>%
             add_class("crtree"))
    }
    ...
    if (lev == "") {
      ...
      if (!lev %in% levels(dataset[[rvar]])) {
        stop(paste0("Specified level is not a level in ", rvar)%>%
               add_class("crtree"))
      }
  ...
  if (type == "classification") {
    ...
    if (!is_not(cost) && !is_not(margin)) {
      ...
      if (any(loss1 <= 0)) {
        ...
        stop("Cost must be larger than zero. Please adjust the settings and try again"%>%
               add_class("crtree"))
      } else {
        parms[["loss"]] <- cbind(loss1, loss2)[, ind]
      }
    } else if (!is.empty(prior)) {
      if (!all(is.numeric(prior))) {
        stop("Prior did not resolve to a numeric factor"%>%
               add_class("crtree"))
      } else if (any(prior > 1 | prior < 0)) {
        stop("Prior is not a valid probability"%>%
               add_class("crtree"))
      } else {
        parms[["prior"]] <- cbind(prior, 1 - prior)[, ind]
      }
    }
  }
  ...
  settings <- settings[!(is.na(settings$prior) & (!is.na(settings$cost) | !is.na(settings$margin))), ]
  ...
  for (i in seq_len(nrow(settings))) {
    ...
    if (!is.null(prior_i) && !is.na(prior_i)) {
      if (!all(is.numeric(prior_i))) {
        stop("Prior did not resolve to a numeric factor")
      } else if (any(prior_i > 1 | prior_i < 0)) {
        stop("Prior is not a valid probability")
      } else {
        parms[["prior"]] <- c(prior_i, 1 - prior_i)
      }
    ...
    if (!is.null(nodes_i) && !is.na(nodes_i)) {
      ...
      if (nrow(model$frame) > 1) {
        ...
        ind <- max(which(cptab$nodes <= nodes_i))
        if (ind > 0) {
          model <- rpart::prune(model, cp = cptab$CP[ind])
        }
      }
    } else if (!is.null(pcp_i) && !is.na(pcp_i)) {
      unpruned <- model
      if (nrow(model$frame) > 1) {
    ...
    if (type == "classification") {
      ...
      metrics[i] <- sum(diag(confusion)) / sum(confusion)
    } else {
  ...
  #environment()$best_model <- best_model
  #environment()$best_model_settings <- best_model_settings
...
summary.crtree <- function(object, prn = TRUE, splits = FALSE, cptab = FALSE, modsum = FALSE, ...) {
  ...
  best_settings <- object$best_model_settings
  for (param in names(best_settings)) {
...
#' @param pcp Complexity parameter to use for pruning
#' @param Random seed to use as the starting point
